...
with st.container():
    ...
    with controls_col:
        ...
        for category, members in CATEGORIES.items():
         ...
         # clean separators from session state before render
        existing = st.session_state.get("species_multiselect", [])
        clean_default = [x for x in existing if x not in separator_labels]
        if clean_default != existing:
            st.session_state["species_multiselect"] = clean_default
